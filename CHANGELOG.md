...
## 2026-08-08 (d56c0dc, 144d05f, dcc82c2)

### 修复问题

- **`core/12-canon-update.md` 要求给每个交手过的 NPC 追一行 interaction history,但两级制
  下大多数 NPC 只是花名册里的一行,没有文件可追。** 现在按等级分流:成卡追进自己文件,
  stub 改写花名册那一行的状态列;写到不存在的文件这个坑因此不会再踩。

- **模板战役的 `sessions/README.md` 教的文件名是错的。** 它写「copy 成 `session-01.md`、
  `session-02.md`」,而 `core/04-design-scenario.md` 与 `core/12-canon-update.md` 两处都
  写 `sessions/<n>-<slug>.md`——照 README 建的备课文件名对不上后面读它的 spec。按
  「`core/` 压过一切」改模板,并补上「场次编号跨幕连续、不重置」这条(此前只写在
  `campaigns/README.md` 里)。
- **模板战役的五份子目录 README 自 2026-07-29 初始 commit 起没动过,已经落后于 kit 本身。**
  `world/README.md` 只字未提 `event-clock.md`——而它是「每个 generator 都读的三份文件」
  之一;地图能力(`scripts/render-map.py`,2026-08-07/08 落地)在模板里完全不存在,
  照模板建目录的人不知道 `.json`/`.svg` 该放哪、玩家版为什么归 `handouts/`;
  `npcs/README.md` 没提每场后必须追一行 interaction history(`core/12` 里是必需步骤);
  `references.md` 的「Kit reference used」漏了 `reference/craft/` 与
  `decks/`+`sourcebooks/` 两块。六份文件一并补齐。
- **地图上的长注释会被画到画布外,看不见。** 渲染器度量不了文字,却把整段注释当一个图元
  画在锚点旁边,只给它留了一格的余量——第一批真实地图(石哨两层)实测 10 条注释有 8 条
  冲出画布被裁掉,剩下的又压在房间名和墙线上;图例过长时同样越界。现在照资料里城区图的
  做法改了:**图上只留一个编号圆圈,注释文字挪到图右侧一栏**,自动折行,注释比图长时画布
  跟着长高——不会再有看不见的注释。**旧地图重跑一次渲染命令**就能拿到新版面,DSL 一个字
  都不用改。
- **intake 的建目录清单漏了 `investigators/`。** `core/01-intake.md` 的 Output 六项里,
  子目录那条只列了 `world/ npcs/ scenes/ puzzles/ handouts/ sessions/`,虽然下一条创建
  `validation.json` 时会把目录隐式建出来,但清单本身不完整;同时写明每个子目录的
  README 要从模板复制过去——那才是各目录惯例(命名、地图、keeper-only 块)的存放处。

### 更新内容

- **生成默认改成"按需",不再一次铺完。** 任何会往 `campaigns/` 写文件的技能(建世界、设计
  模组、造 NPC、造怪物、写谜题、写散文、写讲义、造调查员)开跑前,现在会先给你一份 15 行
  以内的清单——打算写哪几个文件、每份大致放什么、多长——等你确认或删减才动手写。想跳过
  这一步,直接说"全量展开"或"你决定"。
- **NPC 现在分两级:花名册一行(默认),或一整份角色卡(按需)。** 场景里点到的名字先落进
  `npcs/roster.md` 的一行——名字、身份、想要什么、藏着什么、一句口吻;只有当这个 NPC 真的
  要被检定针对、要打斗或追逐、要有一段实质对话,或是你亲自点名要用,才会升级成完整卡面。
  这砍掉了"设计一场戏顺手带出八个人、结果全都要开卡"这笔最大的一次性开销。
- **场景散文默认不落盘,临场再写。** 读给玩家的箱体文本以前默认存成文件;现在默认在对话
  里直接给你,存档只在你说"留着"或它是完整场景网络的一个节点时才发生。场景文件本身仍然
  完整——有什么、能问什么、出口在哪——只是朗读用的正文可以先留 `<pending>`,等真要读的
  那一刻再写。
- **备课时默认只读四份文件,不再是整个 `world/`。** `CLAUDE.md`、`canon-log.md`(近期部分)、
  `world/event-clock.md`、`npcs/roster.md`——四份加起来控制在 400 行以内。`world/` 其余
  文件(地点、势力、时间线)现在配一张"什么时候该打开哪份"的路由表,按需查,不再每次备课
  都整个读一遍。事件时钟本身也改成默认只建前 3 刻,后面的随打随补,不再一次性预支还没发生
  的分支。
- **审查现在分得清"还没生成"和"断链"。** 场景里点到一个还没开卡的 NPC、或矩阵里登记过但
  还没写的讲义,只要它在花名册/线索矩阵里挂着号,就是正常的按需状态,不算问题;真正的
  断链——名字哪儿都没登记——才会被判失败。
- **场景地图现在能带家具,并且能出一份专门给玩家看的版本。** `scripts/render-map.py`
  新增家具层(书架/桌子这类几何图元 + 标签,不是图标)与 `--player` 渲染开关——同一份
  DSL 掷两次:不加 `--player` 是守秘人自己看的版本(暗门、房间真实用途都在);加了
  `--player` 就是能摆到桌上给玩家看的版本(暗门变成实墙、秘密房间与家具要么换成障眼
  说法要么直接消失)。生成玩家版之前,守秘人会先被告知预估 token 成本(比守秘人版贵
  约 3–5 倍)并等你确认,不会自动生成。
- **庄园/野外这类没有房间墙壁的场景现在也能出图,不用再单开一套画法。** 同一个渲染器
  加了两个新图元——折线(小路/溪流)与固定在角落的指北针——`rooms` 留空就是站点图模式,
  家具层的「图元 + 标签」画法原样复用,建议一张图控制在 5–9 个元素,超了就该拆两张。
- 越界或互相压住的家具会被挡下来,不会静默出一张画错的图。
- **玩家版地图的注释编号会重排成连续的,守秘人手上那份自带对照。** 秘密注释在玩家版里被
  整条丢掉,若沿用守秘人版的编号就会出现空号(①③④)——那等于告诉玩家"这儿有一条你没看到
  的"。玩家版因此重新连续编号,守秘人版则在每条非秘密注释后面附一个灰色的 `(PL ②)`,当场
  就能把"看 ③"翻译成玩家手里的号码。另加一条克制规则:**一张图的注释建议不超过 8 条**,
  超了就该拆图或把内容写回正文(脚本不硬拦)。

---

## 2026-08-07 (11f90b0, 86f1335, 7731a01, 090cd3c)
...
- **设计新模组之前,现在会先掷一次它的"形状",不会每次都长成同一副骨架
  (调查 → 对峙 → 解决)。** 新表 `reference/tables/scenario-shapes.md`(1d10):一夜之间、
  通信往来、围城、旅途、倒计时仪式、追缉一人、潜伏卧底、事后回访、分头汇合、循环复现
  ——十个互斥的结构,各自定好时间跨度、调查员站在事件里的什么位置、信息怎么随时间
  到达、结束是什么形态。和 `hooks.md`(为什么是现在)、`mythos-angles.md`(到底是什么)
  分工不同,三者可以同时掷、互相组合。`core/04-design-scenario.md`「Build in this
  order」第一步之前已接线。
