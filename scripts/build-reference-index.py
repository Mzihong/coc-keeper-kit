...
def build_monster_index():
    ...
    lines = [
        ...
        "",
        "> 由 `scripts/build-reference-index.py` 从 "
        "`reference/sourcebooks/malleus-monstrorum-zh.md`(223 条)生成,外加 "
        "`reference/tables/monster-index-data.json` 里手写的「侍奉」与「一句话概要」两栏;"
        "`reference/bestiary/*.md` 里有对应条目时以那边为准(生物一旦被正式写过,那份更权威)。"
        "**不要手改本文件。**",
        ">",
        "> 用它回答\"boss X 该配什么样的精英/仆从\"——看「侍奉」找出谁听命于哪位神格,看"
        "「一句话概要」在同一等级的候选里挑。概要不是揭示文,它是给模型或守秘人做匹配用的,"
        "不是拿到桌上朗读的。见 `core/07-create-monster.md` 与 `core/04-design-scenario.md`。",
        "",
    ]
    for tier_key, tier_title in tiers:
        ...
        lines.append("")
        lines.append("| 名称 | SAN | 侍奉 | 一句话概要 | 出处 |")
        lines.append("|---|---|---|---|---|")
        for r in tier_rows:
    ...
    if extra:
        lines.append("## 套件原创条目(不在 malleus 转录稿里)")
        lines.append("")
        lines.append("| 名称 | 等级 | SAN | 侍奉 | 一句话概要 | 出处 |")
        lines.append("|---|---|---|---|---|---|")
        for b in extra:
