...
**数值可以照取,人物必须自己写。** 卡组的价值在于:一个「古董商」该有多少 EDU、一个
「狂热者」的恐吓开在几点、一条「秘密」写多长才可用——数值与刻度直接取用即可,在
kit 自己的 `reference/` 文件里引用或转录也可以,标明出处就行。
...
`scripts/build-bundle.sh` **不打包本目录**——体量太大,且属第三方文本。用
`dist/bundle.md` 的 ChatGPT/Gemini Keeper 读不到这些卡,生成器必须在没有它们时也能工作:
把卡组当**加分项,不是前置依赖**。
