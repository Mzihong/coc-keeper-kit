# manias.md — 躁狂症状表(Manias)

> 来源:《守秘人规则书》表X 躁狂症状表(5431–5566)

《守秘人规则书》表X 的完整 D100 逐条转录,供直接掷骰查表。**与
`reference/decks/phobias-and-manias-zh.md` 不是同一份材料、不互相重复**:那份文件是
官方卡组的中译收录,只覆盖 16 条躁狂症(带更长的症状描写与游戏效果段落,可直接读给
玩家),是精选的"卡面版";本文件是规则书表格本身的全量转录,给守秘人在需要真正随机
(或从全量列表里手选)一个躁狂对象时查表用,条目简短(一行病理定义),不含卡组那样的
机制段落。两者用途不同,互为补充。

守秘人可投 D100 随机选择,也可直接选一个恰当的症状;选定后记入调查员背景故事栏。
新获得的躁狂症属于疯狂发作的一部分,见 `madness-instant.md`(表VII 10 号)/
`madness-summary.md`(表VIII 10 号);会随场次**习惯化**,见 `reference/rules/sanity.md`
→ 8.5 习惯恐惧。
...
| 4 | 喜高狂 (Acromania) <!-- 待核: 规则书原文此处印作 "Acromaniaheights",疑似 OCR 把英文注释词 "heights" 与拉丁名粘连,未能确认拉丁名原貌 --> | 狂热迷恋高处。 |
...
| 16 | 隐居癖 (Automania) | 过度地热爱独自隐居。(规则书原注:原文如此,存疑——Automania 实际上是"恋车癖"。) |
...
| 23 | 地图狂 (Cartacoethes) | 在何时何地都无法控制查阅地图的冲动。 |
...
| 37 | 毛皮狂 (Doramania) | 痴迷于拥有毛皮。(规则书原注:存疑。) |
...
| 83 | 背德癖 (Planomania) | 病态地渴求违背社会道德。(规则书原注:原文如此,存疑——Planomania 实际上是"漂泊症"。) |
