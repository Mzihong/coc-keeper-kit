...
- **P9 已于 2026-08-03 全部定案(七项),阻塞已解除**——Keeper 同日交付
  `reference/sourcebooks/malleus-monstrorum-zh.md` 重排版转录稿(第十轮),
  17 项执行清单**现在没有任何等待项**,可以从阶段 A 直接开工。
...
- **三份 sourcebook 的手动重译已随第六轮批量提交落地**(9c47d98);误建的空文件
  ...
  的行数字段。malleus 头部的"转录质量"警示已随第十轮换稿重写,不再欠账;
...
### 2026-08-03 — 第六轮:malleus 转录稿换新,P9 阻塞解除

新会话接手,Keeper 一句话交接:"关于 P9,怪物之锤已转录完毕"。工作区里已经躺着上一
会话做完但没提交的两批东西——转载规则二次放宽(数值可转/描述不抄的三分法)+ P9 七项
定案的落盘,以及 Keeper 自己手工替换的 `malleus-monstrorum-zh.md`(8758 行 PDF 提取稿
→ 10113 行重排版稿)。**新稿把文首的收录说明块和文末的 `## 引用出处` 表都冲掉了**——
计划文件「转录稿换新时的三个坑」第 1 条早就点过名,这轮真撞上了。

**做了什么**

1. **提交上一会话遗留的转载规则+P9 定案批次**(8c324a3)——与本轮无关,先落地清账,
   避免和本轮改动混在一个 diff 里说不清楚。
2. **给新版 malleus 重新写头部收录说明与 `## 引用出处`**(`core/14` 步骤 3、4)。
   头部警示改写成如实反映新稿状态:属性表已是 Markdown 表格、数值可直接读,
   不再需要逐条人工修复;残留的小毛病(同义术语两套并存、少量译名不统一、
   十来处残留标记)如实列出但不再是"取数前先判读"级别的警告。
   引用出处表的"本文来源"行如实写"2026-08-03 由 Keeper 手工重排版本替换"。
3. **重跑 `python scripts/build-reference-index.py`**,11 处引用的行号全部刷新,
   报告 no problems。**重跑 `scripts/build-bundle.sh`**,`dist/bundle.md` 同步。
4. **解除 P9 的阻塞状态**:计划文件头状态行从"阻塞"改"待执行",「阻塞范围」一节
   整节改写成「阻塞已解除」;`update_plan/README.md` 的状态索引表、依赖图(去掉
   `KeeperMM → P9A` 这条箭头)、建议执行顺序、复杂度排序表「现在能动」列四处同步。

**为什么这么分**:换稿这件事本身在计划文件里已经预写了操作清单(三个坑 + 正式流程
指向 `core/14`),这轮照着做,没有新增判断;真正需要判断的是头部警示怎么改写——
不是简单删掉旧警示,而是**如实描述新稿的状态**(结构问题解决了,术语不统一还在),
过度乐观的警示和过期的警示同样会误导下一个读数值的会话。

**留下的判断**

- **P9 阶段 A 已在同一会话的下一轮(第七轮)完成**,见下方记录——这里原来写的
  "尚未开始,下一会话接上"已经过时,留着方便对照当时的判断。
- **grand-grimoire、keeper-rulebook 两份 sourcebook 头部警示仍未复核**——这是第四轮
  就记下的欠账,本轮顺手核对了 malleus(已经不欠了),另外两份不在本轮范围内。

### 2026-08-03 — 第七轮:P9 阶段 A(标尺与词条)落盘

紧接第六轮解除阻塞,同一会话继续做阶段 A 全部 8 项执行清单。

**做了什么**

1. **抽样定区间(第 1 项),脚本化而非逐条人工判读。** 计划原文写"不脚本化——转录
   OCR 损坏严重",但那是针对旧的 8758 行 PDF 提取稿写的;新稿属性表已是干净的
   Markdown 表格。改用脚本扫描全书 238 处分类标签(独立种族/仆从种族/唯一存在/
   五类神格各自的小标签),提取每个标签覆盖区间内的 SAN 损失骰式、HP、护甲点数、
   攻击技能百分比,按四类分桶算四分位数,再抽样人工核对分桶结果(如"仆从种族"档
   HP 中位数 15、"神格"档 HP 中位数 63 且有到 421 的长尾)是否读起来合理。
   **产出是数值统计,不是转录文字**——脚本只吐骰式记法(如"1D6/1D20")、百分比数字、
   两三个字的分类标签,没有摘录任何描述性原文。
2. **新建 `reference/rules/monster-scale.md`**(第 2 项)。五级阶梯(human 不进表 +
   creature/servitor/unique/deity 四档),每档四个 threat 子档的 SAN/HP/护甲/攻击技能
   区间,跨级重叠规则,词条负载上限(L2/L3=2、L4=3、L5=4),`core/07` 占位的 X 在文末
   给出完整公式:**怪物强度 = 阶梯基线 + 词条负载,封顶于该级负载上限**。
3. **新建 `reference/tables/monster-traits.md`**(第 3 项)。18 条数值词条,kit 原创
   设计(书里没有这类抽象词条系统,不是取材自转录稿)——再生、分裂、减伤、免疫常规
   武器、抓握、汲取属性、疯狂凝视、恐惧光环、疾病诅咒、多重攻击、连续行动、心灵感应、
   飞行穿地、变形、附身、环境操控等。每条**破解口列强制非空**,负载点 1 或 2。
4. **接线 5 处**(第 4–8 项):`core/07` 的 type 表换成五档 + filing 分工;
   `templates/monster.md` header 加 Tier 字段,Special abilities 改成词条挂载点 +
   负载合计行;`core/02` 登记新 cheat-sheet;`core/11` Mechanics 加两条审计题(词条
   破解口可发现性、负载是否超预算);`reference/bestiary/README.md` 换新分类 tag,
   补古神级条目归 `mythos/great-old-ones/` 的分工。
5. **重跑两个脚本**,`build-reference-index.py` 报 no problems(`monster-traits.md`
   13 处引用被正确收录),`build-bundle.sh` 写出 4973 行/58 文件。
6. **计划文件阶段 A 的 8 项全部勾选**,每项标注实际做法(尤其是第 1 项和计划原文
   预期不同的地方,如实记录不隐瞒)。

**为什么这么分**:第 1 项没有照抄计划原文的"不脚本化"这条约束——那条约束的**理由**
是"转录 OCR 损坏严重,脚本会读出乱码",理由本身已经不成立(新稿是干净表格),照抄
约束但不管理由等于教条主义。抽样统计再加人工核对分桶结果是否合理,达成的是同一个
目标(真实区间、不是估算),没有违背这一条真正想守住的红线。

**留下的判断**

- **`templates/monster.md` 特意没有加 `Serves:` 字段**——那是阶段 B(第 10 项)的
  范围,阶段 A 完成时刻意不越界,避免下一阶段的执行清单出现"其实已经做了"的糊涂账。
- **阶段 A 完成,阶段 B(索引层机制)是下一步**,尚未开始。阶段 B 要改
  `build-reference-index.py`(解析 header 语义字段、生成 `monster-index.md`)、
  回改现有 9 只 bestiary 条目、接通 `core/04` 的检索入口——工作量比阶段 A 更大,
  建议开新的会话窗口做,不在本轮继续硬推。
- **阶段 A 走的是"计划内完结清单"还是"三阶段全完才归档"**:按计划原文,三个阶段
  各自独立交付,但**只有三段全完才归档**——本轮不 `git mv` 计划文件进 `Archived/`,
  `update_plan/README.md` 状态表这一行改成"进行中(阶段 A 已完成,阶段 B 待开始)"。
