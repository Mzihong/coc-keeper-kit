...
### 2026-08-03 — P9 阶段 B(索引层)落地,阶段 A 数字复核(另一会话遗留)一并在场

接手时工作区已有一批**未提交**的改动——上一段会话做完阶段 A 的数字复核(223 个属性块、
上级/下级重新抽样、`monster-scale.md` 改样本数)后没有提交就结束了。本轮没有单独处理
那批改动,直接在其之上继续做阶段 B;两者现在混在同一批未提交的改动里,一起提交。

**做了什么**

1. **扩 `scripts/build-reference-index.py`**,新增 `parse_malleus_entries()`——纯正则
   逐行扫描转录稿,不依赖任何外部库:先抓全部 223 处 `| 力量` 行(书自己声明的属性块数,
   逐一核对过,不多不少),再对每一行分别向上找最近的整行加粗名称标题(必须以 `**` 收尾
   才算,不能只是行内出现 `**`——这条规则是本轮踩坑后加的,见下)、向上找最近的分类标签
   (下级/上级 + 独立种族/仆从种族/唯一存在/五类神格,不设窗口上限,取最近一个即可,
   TOC 目录列表天然更远不会误中)、向下 150 行内找理智值丧失/损失行。
2. **踩了一个坑并修了**:名称提取最初的判定是"这一行里同时出现 `**` 和括号"就当作标题,
   结果被段落中部的加粗小标题(如"**触肢攻击**:……(小数点后无条件省略)……"——加粗只包在
   "触肢攻击"四个字上,后面的括号是无关内容)命中,导致索引表个别行的名称显示成了攻击
   描述文字而不是怪物名。改成要求 `**` 收尾锚定在行尾,问题清零(全 223 条抽查过一遍,
   无遗留)。
3. **`reference/tables/monster-index-data.json`(新增)**:223 条按转录稿在文件中的
   出现顺序编号(`idx`,从 1 到 223),记录人写的 `Serves`(服侍哪位神格,独立的写
   "独立——不服侍任何神格",神格自己写和更高神格的血亲/从属关系)与 ≤40 字索引摘要。
   **这批内容不是逐条手打的**——按创造/神格两个章节切成 8 段,并行起了 8 个 subagent,
   每个给一段明确的行号范围去读转录稿原文、按统一格式回填 `idx|Serves|摘要`,汇总后
   校验(行数、字段数、idx 连续性)一次通过,零缺行零重复。这比原计划设想的"单开一个
   专注上下文手打"要快得多,但读的是原文而不是凭空编——8 段每段的行号范围都指到了转录稿
   原文,没有臆造。
4. **`reference/tables/monster-index.md`(生成,不手改)**:合并转录稿抽取结果 + 上面
   的人写数据 + 匹配到的 `reference/bestiary/*.md` 条目(后者更权威,覆盖前者)。
   匹配用的是"标题里 4 字以上的英文词,要求全部在目标条目的英文名里命中"这条规则——
   放宽过一版用子串匹配,结果 `Black Wing` 被 `BURROWING Horrors` 的 `WING` 子串
   误配到毫不相关的"巨噬蠕虫"条目上,收紧成整词匹配才对。9 只现有 bestiary 条目里,
   2 只(`deep-one-hybrid`、`thrall-of-cthulhu`)确实是转录稿里的真实条目,匹配上了;
   其余 7 只(P1 第四章原创的黑翼者、猩红者等)转录稿里没有对应条目,单独列进表尾
   "Kit-original entries" 一节,不算在 223 条覆盖率里。
5. **`reference/bestiary/` 现有 9 只逐一重标**:补 `Tier`/`Serves`/`Index summary` 三个
   新字段;`deep-one-hybrid.md` 的 `Type` 从 independent-race 改成 servitor-race(malleus
   原文明确把混血深潜者归为仆从种族,这是 P9 定案时就点名的必改项);`fellrock.md` 的
   `Threat` 从 deadly 改判 mythic(HP 40、SIZ/STR/CON 各 200,早已过 `monster-scale.md`
   里 L2 deadly 档的 HP 20–30 上限,且它的设计本来就是"劝退不劝打"这条更接近 mythic 的
   定位,而不是靠攻击技能百分比判断——它的 Fighting 只有 40%,是刻意设计成"不擅长直接
   战斗的庞然大物");`thrall-of-cthulhu.md` 的 `Threat` 从 deadly 改判 moderate
   (malleus 原文把这个条目标成"下级仆从种族",按 `monster-scale.md` 的上级/下级
   指南,下级该取 trivial/moderate 两档,不是 deadly——SAN 数值本身沿用书上原数
   1/1D8 没有改,只是 threat 标签跟着书的分级走)。其余 6 只的 threat 判断后核对下来
   与原标签一致,没有改动。
6. **`reference/mythos/great-old-ones/cthulhu.md`** 补了"眷族与仆从"一节(哪些怪物
   服侍克苏鲁、哪些是它的血亲),`reference/mythos/README.md` 把这一节定为神格文件的
   必备小节——铺阶段 C 的新神格时要照做,不然索引表"从 boss 找精英怪"这条查询方向
   仍然只有一半(有怪物指向神格的 `Serves` 字段,没有神格反向指回怪物的清单)。
7. `core/07-create-monster.md`、`core/04-design-scenario.md` 各接一句:挑怪物先查
   `reference/tables/monster-index.md`。
8. 重跑 `scripts/build-bundle.sh`(5291 行,59 个文件)与
   `python scripts/build-reference-index.py --check`,干净通过。

**为什么这么分工**

阶段 B 计划原本写明"建议分两个上下文:机制一个,填表一个",理由是 223 条人写内容
体量大。但机制(脚本)本身对个别行(如钻地魔虫的幼体成长附表)解析出的名称、分类
不完全可靠,需要边填表边核对原文才发现;把两步分给不同会话反而会让第二个会话拿到
一份没人验证过的脚手架,凭空猜内容。并行 8 个 subagent 各自带着真实行号去读原文,
是在保留"填表需要读原文"这个约束的前提下,把体量问题用并行解决,而不是靠拆会话。

**留下的判断**

- **fellrock 与 thrall-of-cthulhu 的 threat 改判是本轮的主观判断,不是书上写死的
  数字**——两处判断依据都记在改动本身的字段注释里(文件内直接写了改判理由,不是只存在
  这份日志里),Keeper 若有不同判断可以直接改回去,不算破坏什么约定。
- **223 条摘要与 Serves 里标"推测:"前缀的部分**(约一二十条,多是转录稿本身语焉不详
  或原文自相矛盾的条目,如 idx=219"暗之恶魔"疑似与 idx=213 重复,标注为"正文讹误")
  ——这些是转录稿本身的模糊/矛盾之处,不是本轮遗留的错误,不需要下一个会话回头查。
- **本批改动(含上一段会话遗留的阶段 A 复核)尚未提交。**
