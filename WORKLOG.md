...
### 未提交:P12 阶段 2 + eras/ 语言体例统一(工作区,2026-08-06)

阶段 2 四份速查表(`skill-checks.md`/`character-creation.md`/`magic.md`/
`monster-scale.md`)整份重写,细节见 `update_plan/2026-08-04-cheatsheet-self-sufficiency.md`
阶段 2 小节与本文件上面"当前状态"里的三条摘要。这里只记两件容易漏的:

- **`build-reference-index.py` 与 `roll.py --check-all` 都已在改动后重跑,通过。**
  `reference/rules/index.json` 与 `reference/tables/index.json` 已是重建后的版本。
- **`reference/glossary-zh.md` 补了检定补充(组合技能检定、人体极限、暗骰、熟人、训练、
  老化等)与神话补充(泛读/精读、CMI/CMF/MR、深层魔法、相信者/不信者)两批新词**,
  按字母序插入既有分组,没有新开顶级分类。
- **`core/02-rules-reference.md` 的 Cheat-sheets 清单与 Fast facts 已同步四份新内容**,
  并把阶段 1.3 复审总结出的"逐条核数值,不许凭印象写"那条硬约定正式接了进去(此前只
  写进了 `reference/rules/README.md`,`core/02` 一直是待办)。
- **`update_plan/README.md` 的状态索引与完结清单尚未走完**——下一步收尾时记得把 P12
  状态改成"阶段 3"或"已完成",并过一遍完结清单七项。

### 未提交:P12 阶段 1 + 2026-08-06 复审(工作区)

P12 阶段 0 与阶段 1(1.0–1.3)整批尚未提交,细节见计划文件本体。**这里只记复审那一段,
因为它改的是「怎么验」而不是「验了什么」:**

- **"抽查三条数值"这个验收方式不成立,别再照抄。** 阶段 1.3 抽的三条恰好是本批**改掉的**
  三个已知错误,属于回归检查;**同批新写、没被改动过的数值一条都没进抽查名单**。逐条
  回原文核完之后又查出 5 处错:`sanity.md` 疯狂发作时长(定值 `10 轮` → `1D10 轮`)、
  `combat.md` 全自动难度阶梯(四组对应关系被 `→` 串成七级链)、`skill-checks.md` 对抗
  检定平手规则(整条是编的)+ 孤注一掷限制写窄 + 幸运值四类禁用场景漏写。
- **最阴的是那条难度阶梯。** 原文"普通变困难,困难变极难,极难变大成功,大成功变不可能"
  写成 `普通→困难→困难→极难→极难→大成功→大成功→不可能` 之后,**每个词单独看都在原文
  里出现过**,只有整体语义错了。逐词 diff 查不出这种,必须核**关系**。
- **新硬约定已写进 `reference/rules/README.md`**(该目录此前根本没有 README,而
  `rules/index.json` 的 `governed_by` 一直指着它;`build-reference-index.py` 不校验
  `governed_by`,所以索引一直报干净——**这个校验缺口本身也值得记一笔**)。
  阶段 3 还要把同一条接进 `core/02`。
- **自足性回收 13 处**:速查表既然是"最后一句话",里面就不能留把人踢回规则书的出口。
  `combat.md` 5 处裸「表XVII」、`chases.md` 2 处裸「表III」、6 处规则书页码/章节指路,
  已分别改指仓库对应物或把规则本身写进来。`chases.md` 7.5 的锚点重叠也拆了。

### 未提交:`update_plan/README.md` 整份核对(2026-08-06)

顺着上面那条"三份日志改了、状态索引没改"往下查,把跟踪表整份对着六份计划正文与仓库
实际状态核了一遍,**7 处失准全部修正**。**共同形状:一个事实变了,复述它的第二处没跟着变。**

- **`建议执行顺序` 第 1 条整条过期**,而且在建议一个**已经不存在的动作**——推迟
  「阶段 1.2 `git rm --cached` 规则书」。那三项已随「已定案 ②」于 2026-08-04 晚整体取消。
- **依赖图欠一次删箭头**(`P15 ⇢ P12`):P12 阶段 1.0 落地后 P15 问题 2 已解,索引表那行
  早写着"无阻塞",**图和表互相打架**。完结清单第 6 项本来就要求删。
- **P5 阶段 0 的 hash 两处都没回填**:产物其实早随 `13821ac`(2026-08-04)提交了,
  索引表与计划文件(头 + 阶段 0 尾注)却都还写"待提交回填 hash"。
- **P15 的 `core/NN:行号` 漂了三处**(`core/00:107→114`、`:173→189`、`core/13:83→92`),
  分别是被 P12 阶段 1.0 和 P11 阶段 2b 改动带偏的;`core/00:103` 引的那句更是已被 P12
  整句删除。**P15 自称"逐条核对了 16 份 spec 的全部路径引用",行号错了这句就不成立。**
  已重锚并照 P12 的先例加了「行号会漂,别直接 sed」的警示。
- 另三处:P12 计划头与索引表状态不一致(完结清单第 1 项)、复杂度表「工作量实测」栏把
  `roll.py` 写成 ~200 行(实际 310)、P14 计划头不符本文件规定的 `状态取值` 格式。

**制度缺口:完结清单只有第 1 项管"状态两处同步"、第 7 项管 hash 回填,
依赖图/建议执行顺序/复杂度表这三处复述没有任何一条清单管。** 这正好是 **P15 阶段 3
三条制度动作**要治的东西,接手做 P15 时可以拿这 7 条当实证样本。
