...
> 日期:2026-08-04
> 状态:**已完成(阶段 0 `11f90b0`;阶段 1-3 `86f1335`)**——阶段 0-3 全部完成;两个拍板点已由 Keeper 于
...
### 7.(已并入 P11 阶段 2b,并已于 2026-08-05 随 P11 阶段 2b 解决,尚未提交)路径 C 的 Era 字段没有合法值
...
- [x] **先做一次 P13 余波扫描**(问题 1 揭示这不是孤例):
      `grep -rn "bundle\|ChatGPT\|never ships\|does ship\|拿不到\|无文件系统"` 全仓,
      **人工读每一处命中**,不看计数。已知至少四处:
      - [x] `core/07:14-15` —— 删掉「only channel / never ships / does ship」整段框架,
            改写成索引的真实定位(检索层——两者都在仓库里,索引指路,读原文核对)
      - [x] `core/00:114` —— 见下面 `external/` 那条
      - [x] **P5 阶段 3** —— 整节假设存在"无文件系统的 ChatGPT 网页链路",
            而 `core/00:189` 已写死 kit 由**有文件系统的 agent 就地读**。
            **这一条不在本计划范围内改,但要把结论报给 P5**:该链路若已不存在,
            阶段 3 连同"写进 `core/00` 的链路差异说明"可能整块不必做。
            **2026-08-06 已作为提示框写进该计划文件阶段 3 开头**,只报不改
      - [x] **P10 复评修订 A 的第一条理由** —— "不进 bundle 等于目标用户拿不到"已失效。
            **结论(§六 落在 `diagram-conventions-zh.md`)不变**,另外三条理由站得住,
            只是这条理由要换。**2026-08-06 已作为提示框写进该计划文件对应段落**,同样只报不改
      > **P5 阶段 3 的存废影响 P10 建筑卡那张的存废理由**(P10 修订 B 说"砍掉它
      > ChatGPT 链路就彻底没有室内地图")。**两者要一起确认,不要分两次答。**
- [x] `core/00:103`(现已随 P12 阶段 1.0 漂到别处)指向
      `update_plan/2026-08-02-monster-templates-traits.md` 的死链 ——
      **复查:2026-08-06 全文 grep `monster-templates-traits` 命中 0 处,已随 P12 阶段 1.0
      整句删除时一并消失**,本条不需要再改,只确认
- [x] `WORKLOG.md` 硬约定 5 里同一句「过渡期边界(等 P9 定案)」——
      **复查:2026-08-06 全文 grep「过渡期边界」/「等 P9 定案」命中 0 处,已随 P12 消失**,
      只确认不需要再改
- [x] `reference/external/` 三处一起处理(**先核实:`git ls-files -s | grep 160000`
      与 `git submodule status` 均为空,磁盘上 `reference/external/` 目录本就不存在
      ——子模块早已被摘掉,只剩配置残留,不构成"是否还要"的取舍,直接清残留):
      - [x] 删 `.gitmodules`(唯一段落就是 `reference/external/coc-zh`,删段落后是空文件,
            整份删除)
      - [x] 改 `core/14:38` 分类表那一行为"第三方 git repo:当前不收,要收先新建目录并给
            README"
      - [x] 改 `core/00:114`,只保留 `reference/craft/`
- [x] `core/07` Output 段补 L5 例外(→ `reference/mythos/great-old-ones/`),
...
- [x] `core/00` Layout 树补 `mythos/spells/`(**2026-08-06 已补**,`mythos/` 那行下面
      加一行子目录说明)
...
2. **问题 7 与 P11 阶段 2b 是同一条缝** —— 见问题 7。**P11 已先做完**(2026-08-05,
   未提交),收口位置落在了 `core/02` 而不是 `eras/README.md`,按建议定案。
