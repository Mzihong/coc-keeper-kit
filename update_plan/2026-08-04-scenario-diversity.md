# Update Plan — 模组多样性:掷骰走脚本 + 补缺表 + 提高掷骰频率

> 日期:2026-08-04
> 状态:进行中(阶段 1 `scripts/roll.py` + 硬约定接线已完成并提交,148bb91,2026-08-05;
> 阶段 2 种子表 bug 修复 + locations 接线、阶段 3 `confrontation-grounds.md`、阶段 4
> `scenario-shapes.md` 均已完成,2026-08-07,阶段 4 待提交;阶段 5–6 待执行)
> 来源:Keeper 2026-08-04 会话——先问「1d20 会不会太少了,能不能拓展 1d100」,
> 同日重述为「**想要 100 条的根本原因是希望做出来的模组具有多样性,否则每个模组都发生在
> 类似场景中战斗未免太枯燥无味**」,并追加一条硬要求:**掷骰必须用 `.py` 取伪随机,
> 不能让大模型决定**
> 前置:无。与 P12 有软耦合(都动 `reference/tables/`,别同批 commit)

## 这份计划换过一次方向

**原方向(已推翻)**:把 `reference/tables/` 的表从 d20 扩到 d100,约 390 条新条目。

**推翻的理由是它的前提在数学上不成立。** 四张种子表现在是 20⁴ = **16 万种组合**,
扩到 d100 是 1 亿。守秘人这辈子跑不到 160 场团——**"组合空间被 20 条封顶"不是真的**。
如果实际感觉每个模组都像同一个,原因一定在别处,写 320 条不会解决它。

原方向里唯一站得住的观察是**"20 条会被很快摸熟"**。但那是**采样问题不是长度问题**——
详见下面的诊断第 ①、③ 条。老约定(`tables/README.md`「宁可 20 条具体的,也不要 100 条
含糊的」)因此**不必推翻,原样保留**;新方案是多开几张 20 条的表,不是把一张表灌到 100。

## 目标

**让两件事变得不一样:模组与模组之间,以及一个模组内部的场景与场景之间。**
不以骰面数为目标,以「掷骰真的均匀」「该掷的地方有表可掷」「掷得够勤」为目标。

## 诊断:多样性漏在哪四处

### ① 模型掷骰不是均匀的(**最上游,也是 Keeper 点名要修的那条**)

`core/01-intake.md` 说 "Roll; don't pick the first plausible thing you think of",但没有任何
机制保证这件事——模型口头报的点数会偏向表里写得最勾人的那几条。**这一条不修,后面三条
全部打折**:扩到 100 条的结果只是它从 100 条里挑同样那 10 条,摸熟速度分毫不变。

### ② 真正雷同的那个维度,现在的条目数是 0 不是 20

Keeper 点名的「每个模组都在类似场景里战斗」——**kit 里没有任何一张表掷对抗场面**。
[`core/04-design-scenario.md:40`](../core/04-design-scenario.md) 给场景分类是
clue / choice / shock / breather,**战斗连一个节点类别都不是**;`tables/` 十三张表里没有
一张管地形、双方数量、环境里能用的东西、为什么跑不掉。于是每次都是现场即兴,而即兴的
对抗场面必然收敛到仓库 / 地下室 / 夜里的码头 / 一群邪教徒围上来。

**从 0 写到 20 条的收益远大于把 `locations` 从 20 写到 100**,成本是 20 条对 80 条。

同一类缺口还有一处更上游的:**模组的形状**。`core/04` 的 "Build in this order" 每次产出
同一副骨架,而「一夜之间 / 三个月的通信往来 / 围城 / 旅途 / 倒计时仪式 / 只追一个人」
这一维**从来没有被掷过**。

### ③ 高频表比种子表更容易被摸熟,而原计划的分层正好是反的

| 表 | 一个战役掷几次 | 原计划优先级 |
|---|---|---|
| `npc-quirks` | 每造一个 NPC 一次,**8–15 次** | 种子表,排第 4 |
| `complications` | 每场两次,**6–10 次** | 降级 d50 |
| `locations` | **1 次**(接线后 8 次) | 种子表,排第 2 |
| `hooks` / `mythos-angles` | **各 1 次** | 排第 1、第 3 |

`mythos-angles` 一个战役掷一次,要跑 20 个战役才轮完一圈;`npc-quirks` 两个战役就开始重复,
而且是**在同一个战役内部**重复——那才是牌桌上真能感觉到的雷同。

**算一下**:20 条均匀掷 10 次,出现重复的概率是 **93.5%**(生日悖论)。
换成 d50 是 61.8%,d100 仍有 37%。**靠加长条目治撞车,性价比极低**——
真正治它的是**同一战役内不放回抽样**,见阶段 1。

### ④ `locations` 一个战役只掷一次,扩它只改那一掷

[`locations.md:6`](../reference/tables/locations.md) 自己写着"之后场景需要新地点时再掷",
但 `core/04` 建 scene web 时**没有接这条线**。一个战役掷一次,剩下 8–10 个场景的地点全是
自由创作。**接线是零写作成本的收益**:掷骰次数从 1 涨到 8。

## 拍板结果(2026-08-04)

- ① **不推翻**「宁可 20 条具体的」那句约定——新方案与它不冲突。
- ② **不做全面 d100**。扩容降级为阶段 5 的条件执行项,判据见该阶段。
- ③ **新增硬要求(Keeper)**:掷骰一律走 `scripts/roll.py`,模型不许自己报点数。

## 阶段 1 — `scripts/roll.py`,以及"不许口头掷骰"的硬约定

**这是全计划的地基,先做。** 它一次修掉诊断 ①③ 两条,并让阶段 5 变成可选的。

### 1.1 脚本

- [x] 新建 `scripts/roll.py`。用 `random.SystemRandom()`,**不接受模型给定的点数**。

```
python scripts/roll.py <table> [<table> ...] [--times N] [--campaign <slug>] [--fresh]
```

- [x] **解析表文件自己声明的骰面**:实现取的是 markdown 表格本身的 `dN` 表头
      (多维表另外核对小节标题的 `(1DN)` 与表头是否一致,不一致直接报错)——比只认开头那句
      `掷 **1dN**` 更稳,骰面终归是从表结构读出来的,不是从参数传的。
- [x] **两维表原生支持**:`cult-goals.md` 的 `## 愿望(1D10)` / `## 手段(1D8)`
      各掷一次,一并输出;已用真实文件测试通过。
- [x] **输出是可原样粘贴的整行条目,不只是点数**(格式与本节示例一致,已实测)。
- [x] **`--campaign <slug>` 记账**:追加到 `campaigns/<slug>/rolls.log`(tab 分隔:表名 /
      维度 / 骰面 / 点数 / 条目 / spec / 时间戳)。新增 `--spec <name>` 可选参数供调用方标注
      是哪个 spec 掷的;campaign 目录不存在时脚本会自己建一个空文件夹再记账,不强制先跑完
      intake 的六项产物(解了风险 3 的一半——见下)。
- [x] **同一战役内不放回**(给了 `--campaign` 即默认开启)。实现是集合差(可用点数 = 全部
      点数 − 已用点数)而不是字面的"重掷循环"——效果等价且不会浪费尝试次数,表掷空时
      落回全范围随机并在输出行末尾注明。已用 `--times 20` 实测:20 次不重复,第 21 次准确
      提示"已掷空,允许重复"。
- [x] **`--fresh` 跨战役查重**:同样用集合差实现(而不是"最多重试 20 次"的循环),
      跨 `campaigns/*/rolls.log` 收集已用点数再排除。已用两个测试战役实测:另一战役已把
      20 条全部掷完时,`--fresh` 立刻精确报告"已掷空",不需要真的retry 20 次去撞。
- [x] **非骰表要报错不要硬掷**:`cultist-archetypes.md` / `monster-traits.md` /
      `monster-index.md` 三个走显式排除名单(`NOT_DICE_TABLES`),各自带一句不掷骰的理由。
- [x] **自检:每张表的实际行数必须等于骰面数**,不齐就非零退出——且是"先全部校验、
      一个不过就整体不掷",不会出现掷了一半才报错的半成品记账。额外加了
      `--check-all`(不在原清单内的加项):不掷骰,只把 `reference/tables/*.md` 全部过一遍
      自检,真正补上 `build-reference-index.py` 的那个盲区,而不是等某张表被掷到才发现
      行数不对。已跑通:10 张骰表通过、3 张非骰表正确跳过。
- [x] `--seed` 仅供测试,**不写进任何 spec**——用时脚本自己在 stderr 打一行警告提醒。

**顺带修的一个平台坑**:Windows 终端默认代码页不是 UTF-8,脚本打印中文会变成乱码
(不影响 `--campaign` 写文件,`rolls.log` 一直是显式 `encoding="utf-8"`,只影响屏幕
显示)。加了三行 `sys.stdout.reconfigure(encoding="utf-8")` 处理,非 Windows 环境是空操作。

### 1.2 硬约定与接线

- [x] `core/00-how-to-run.md` → ground rules 增一条(与 "Fair play" 同级)。
- [x] `CLAUDE.md` / `GEMINI.md` / `AGENTS.md` **三份同步**(硬约定 1:只在一份里就是 bug)。
- [x] 逐处改掷骰点的措辞:`core/01-intake.md` auto-fill 与反套路节、`core/04` 的
      complications、`core/06-create-npc.md` 的 npc-quirks(顺带同段的 npc-appearance /
      cult-leader-positions 一并改了措辞)、`core/03` 的 cult-goals(顺带同段的
      cult-leader-positions)。
- [x] `reference/tables/README.md` 顶部加一句"怎么掷"。
- [x] `core/11-review.md` 增一条审查项:**战役里找不到 `rolls.log`,或产物里的种子对不上
      日志,判 fail**。

**风险 3 留下的另一半**(`_template-campaign/` 要不要预置空 `rolls.log`、要不要进
`core/01` 的 Output 六项清单)本轮定案:**都不做。** 空文件本身没有信息,`roll.py`
在第一次真正掷骰时会自己创建 `rolls.log`(连同 `campaigns/<slug>/` 文件夹一起,如果还不
存在),这就绕开了 intake 里"掷种子表在先、建文件夹在后"的时序问题,不需要模板预置或
清单强制。六项清单也不动——一个完全靠 Keeper 手填、零随机的 intake 合法但不会产生
`rolls.log`,把它列为必需项会让这种(允许的)路径被清单误判成不完整。

## 阶段 2 — 零条目的接线(先修 bug,已完成 2026-08-07)

- [x] **修:种子表到底是哪四张,两份文件不一致。**
      `reference/tables/README.md` § 种子表说是 hooks / locations / mythos-angles /
      **npc-quirks**,并断言「`core/01-intake.md` 强制要求掷这四张」;
      而 [`core/01-intake.md:124`](../core/01-intake.md) 实际写的是 hooks / locations /
      mythos-angles / **complications**。
      两份从同一个 commit(`d213f2e`)一起出生就不一致,各按一套心智模型分类
      (README 按「什么让战役具体」,intake 按「auto-fill 当场需要什么」),`13821ac`
      同时改过两份仍没对上。**按硬约定 `core/` 压过一切 → 以 intake 那版为准**,
      改 README,并把那句失实的断言一起改掉。**实际落地**:种子表四张改成
      hooks/locations/mythos-angles/complications;`npc-quirks.md` 从种子表节移到
      「备课与临场表」节(它本就不在 intake 的四张之列,只是高频表——见 P14 诊断③),
      并顺手挪到紧邻它的姊妹表 `npc-appearance.md` 前面。`complications.md` 的条目从
      「备课与临场表」并入种子表节(它现在两处都掷:intake 定基调 + 单场备课再掷),
      不重复列两遍。
- [x] **接线:`locations.md` 掷到场景级。** `core/04` 第 6 步建 scene web 时,每个新地点
      掷一次(表自己第 6 行早就这么写了,只是没人读它)。**零新条目,掷骰次数 ×8。**
      **实际落地**:`core/04-design-scenario.md` 第 6 步新增一句——场景需要的地点若
      `world/` 里没有,当场跑 `python scripts/roll.py locations --campaign <slug>`。
      `reference/tables/locations.md` 自身的说明行同步补了 `core/04` 第 6 步的指路
      (原来只指到 `core/03`,读者看不到场景级这一半)。
- [ ] **遗留欠账(来自 P16,2026-08-07 发现,未修):** `clue-engines.md` 落地时给
      `core/04-design-scenario.md` 第 5 步新增了一处掷骰点,但阶段 1.2「逐处改掷骰点的
      措辞」的清单当时没有这张表(还没造出来),那处措辞至今仍是 "roll
      `reference/tables/clue-engines.md`"——不是本计划别处已统一的
      `python scripts/roll.py <table> --campaign <slug>` 可执行形式。下次改 `core/04` 第 5
      步时顺手补上;不阻塞阶段 3–6。

## 阶段 3 — 新表 `confrontation-grounds.md`(d20)(已完成,2026-08-07)

**直击 Keeper 点名的那件事。** 掷的是**对抗场面成立的条件**,不是"打不打得起来":

- 地形与它带来的限制(高低差、水、暗、窄、易燃、人多)
- 场上有什么能用、能推、能烧、能挡
- **为什么跑不掉**,或者跑掉的代价是什么
- 场面结束的条件——不是"打死",是"什么发生了这一场就结束"

- [x] 写 20 条,一条 1–2 行,年代无关(约 30 行)。**实际落地**:格式仿照 `clue-engines.md`
      的多列结构(`d20 | 场地 | 地形与限制 | 场上能用的 | 结束于`),不是单格长句——
      `scripts/roll.py` 的表格解析本就支持任意列数按 `— ` 拼接输出,四个维度分列比塞进一句
      话更好读也更好维护。已通过 `--check-all`(20 行对 d20)与 `--seed` 实测三次抽样。
- [x] 接线:`core/04` 第 6/7 步(第 6 步加物理对抗掷这张表、与 `chases.md` 的边界一句话;
      第 7 步把怪物的"fair out"接到掷出的结束条件上)、`core/09-description.md`(Mode A
      "Everything above" 后加一条:对抗场面的空间描写要照掷出的地形/可用物写,不凭空造)、
      `tables/README.md` 表清单加一行。
- [x] 与 `reference/rules/chases.md` 划清边界:那份是**追逐的机制**,这张是**场面的选材**
      ——文件头与 `core/04` 接线处各写了一句。
- [x] 自检:有多少条只在一个年代/一个国家成立。**结果**:20 条里 2 条(#3 卷筒机印刷厂、
      #19 钢架施工楼顶)预设工业化之后的年代,其余 18 条(谷仓/礁石滩/太平间/灯塔/市集/
      矿井/墓地/磨坊/温室……)不挑年代;**0 条挑国家**,没有任何条目点名具体国名或只在
      单一地域成立。两条工业化条目保留不砍——`locations.md` 本身也收了"铁路枢纽"这类
      同等级的条目,且比例(10%)不影响表的可用性。

## 阶段 4 — 新表 `scenario-shapes.md`(d10–d20)(已完成,2026-08-07)

**"每个模组都像同一个"最上游的那一处。** 掷模组的**形状**:时间跨度、调查员的位置
(受雇 / 被困 / 目击者 / 已经在里面了)、信息到达的方式、结束的形态。

- [x] 先写 **5 条试水**,确认这一维不是在重复 `hooks`/`mythos-angles` 已经掷过的东西
      ——这张最容易写虚,写不出区分度就砍掉本阶段,不硬做。**实际落地**:试水的 5 条
      直接取自本计划诊断②里 Keeper 原话点过的六个例子(一夜之间/通信往来/围城/旅途/
      倒计时仪式/追缉一人,选了其中五个),核对后与 `hooks.md`(触发这件事的具体导火索)
      和 `mythos-angles.md`(那份"不对劲"的本质)不重叠——scenario-shapes 答的是"整段
      模组怎么随时间铺开",三者可以同时掷、互相组合,不是同一件事,判定通过。
- [x] 通过则补满,接线到 `core/04` 第 1 步之前(形状先于骨架)。**实际落地**:补满到 10 条
      (追加潜伏卧底/事后回访/分头汇合/循环复现四个原创形状),定为 **1d10** 不是 d20——
      模组形状是比场景选材更宏观的一层决定,10 个真正互斥的形状比硬凑 20 个更站得住,
      判断依据同 `clue-engines.md` 当初选 d10 的理由。格式沿用 `confrontation-grounds.md`
      的多列结构(`形状 | 时间跨度 | 调查员的位置 | 信息怎么来 | 结束形态`)。接线:
      `core/04-design-scenario.md`「Build in this order」标题后、步骤 1 之前新增一段
      (设计新模组/新支线前掷;给现成模组备场次不必重掷,形状在设计时已定)、
      `tables/README.md` 表清单加一行。已通过 `--check-all` 与 `--seed` 抽样验证。

## 阶段 5 — 只扩高频表(条件执行)

**判据:阶段 1 的不放回落地后,先跑一个真战役,再决定要不要扩。**
只有当**一个战役实际用到的条数 > 表长的一半**时才扩那张表(此时不放回已接近掷空)。

- [ ] `npc-quirks.md` → d50(按上面的口径,一个战役 8–15 个 NPC,最可能触发)
- [ ] `complications.md` → d50
- [ ] `hooks` / `mythos-angles` / `locations` **暂不扩**——一个战役各掷 1 次,靠 `--fresh`
      跨战役查重就够,扩了也摸不熟

## 阶段 6 — 收尾

- [ ] `reference/tables/README.md`:「怎么掷」一节、表清单更新、种子表四张改对
- [ ] `README.md` 快速开始:python 依赖从"渲染卡面用"变成"掷骰也要用",要写明
- [ ] 重跑 `python scripts/build-reference-index.py`
- [ ] `CHANGELOG.md` + `update_plan/README.md` 完结清单七项

## 风险与边界

1. **模型可能绕过脚本。** 这是本计划最大的执行风险——`core/` 里写"必须跑"不等于它会跑。
   挡法有三层:产物里必须附命令与原始输出、`rolls.log` 落盘、`core/11` 判 fail。
   三层都是事后可查的,不依赖自觉。
2. **python 不一定在。** kit 已经依赖 python(`render-investigator.py`),但掷骰是**每次
   生成都要用**,依赖等级比渲染卡面高一档。降级路径必须写死:**请守秘人手掷,不许自己编**。
   降级发生时要**在产物里注明**,否则事后分不清哪次是脚本掷的。
3. **`rolls.log` 是新的战役级文件**,`campaigns/_template-campaign/` 要不要预置空文件、
   要不要进 `core/01` 的 Output 六项清单——阶段 1.1 落地时一并定。
4. **凑数就是失败**(从原方案保留):新表宁可 20 条具体的。阶段 4 的"5 条试水"就是这条的
   实证检验,写不出区分度就砍掉整个阶段。
5. **和 P12 的边界:`madness.md` 不归本计划**,它是规则书表VII 的转录,长度由书决定。
   两个计划都动 `reference/tables/`,**别同批提交**。

## 被推翻的原方案(留档)

原计划的主体是「四张种子表 → d100 / 备课表 → d50 / 合计约 390 条 ~600 行」,并要求先推翻
`tables/README.md` 那句成文约定。**推翻理由见本文开头**:组合空间从来不是瓶颈,而它真正
想治的"摸熟"是采样问题,由阶段 1 的脚本以近乎零写作成本解决。原方案里同时考虑过的
「乘法路线」(两张 d10 相乘 = 100 组合)一并作废——它要求两维真正正交,硬拆出来的废组合
会给 `core/01` 的"take what you get"开后门,而现在也不再需要用它省成本。
