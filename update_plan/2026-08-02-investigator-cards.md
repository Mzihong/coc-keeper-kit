...
> 日期:2026-08-02
> 状态:进行中(基线 e0d026b;第二轮「照真实车卡表重建 schema」已交付;
> 剩跨 P1 复用 1 条 + `roster.csv` 可选项 1 条)
...
- [ ] **跨 P1:** 教团计划第四章的邪教徒精英用同 schema 存档(NPC 用满卡时)——
      留给 P1 第四章执行时使用,此处 schema 已就绪,不需要本计划先动
...
- [ ] `roster.csv` 花名册索引:一行一人的 name/occupation/player/status,
      供多人桌快速点名。选型时列为"可选",至今未做——人少时纯属多余,
      要不要做等有真实需求再说
