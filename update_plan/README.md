...
| P14 | [tables-d20-to-d100](2026-08-04-tables-d20-to-d100.md) | kit 自己的随机表 d20 → d100(四张种子表优先);约 390 条新原创条目 | 待讨论(方向要拍板) | **和现行成文约定正面冲突**——`reference/tables/README.md` 写着「宁可 20 条具体的,也不要 100 条含糊的」。要扩就得连那句一起改并写明新判据。另有一条更便宜的替代路线(乘法:两张 d10 相乘 = 100 组合,只写 20 条),逐表判断 |
...
graph LR
    ...
    P14["P14 随机表 d20→d100<br/>(等拍板:与现行约定冲突)"]
...
P13(bundle 退役)当日执行完毕并归档。** 现在唯一还在等拍板的是 **P14**——它要推翻
`tables/README.md` 里一条成文约定,不是执行细节而是方向问题。其余四条计划都没有硬阻塞。
...
9. **P14 拍板**(随机表要不要从 d20 扩到 d100)——它要推翻 `tables/README.md` 里
   「宁可 20 条具体的」那句成文约定,是方向问题不是执行细节。**拍板前可先做阶段 0 的
   「每张表试写 5 条」**——那一步本身就是对那句老约定的实证检验。
...
| 5 | **P14** 随机表 d20→d100 | ★★★★☆ | 约 390 条**原创**中文条目 / ~600 行,分 7 张表;外加 `tables/README.md` 一条约定改写 | **本仓迄今单次写作量最大的一项,且没有原文可对**——P10/P11/P12 都是「照着资料写」,这一项是纯创作,质量全靠人判断。★ 高在这里,不在技术。**凑数就是失败**:一张 d100 里混 30 条同义条目,比 d20 更差。挡法是阶段 0 先每表试写 5 条 | ⚠️ 阶段 0 可动,阶段 1 等拍板 |
