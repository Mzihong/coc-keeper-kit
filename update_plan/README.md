...
**P15(`core/` 复查勘误)已于 2026-08-07 全部完成,待归档**,不再是图上的节点——
`P16 -. 阶段 4 与 P15 阶段 2 撞 core/11 .-> P15` 这条箭头已随 P15 阶段 2 与 P16 阶段 4
双双落地(顺序 P11 → P15 → P16,无冲突)一并删除。

**P16(线索引擎 + 三线索检验)已于 2026-08-07 全部完成,待归档**,不再是图上的节点——
`P16 == clue-engines 是新骰表,该一出生就接 roll.py ==> P14` 这条实线随之删除;
`clue-engines.md` 已在落地时直接接了 `roll.py` 并通过 `--check-all` 自检,这条耦合
描述的风险(新表先脱离脚本被用熟,回头要改接线)没有发生。**遗留欠账**:`clue-engines.md`
新增了 `core/04` 第 5 步一处掷骰点,`P14` 阶段 1.2「逐处改掷骰点的措辞」的清单里没有
这一处——`P14` 做到阶段 1.2 时记得一并补上(与 P10 遗留的两处 `core/03` 掷骰点同一笔账)。
...
**P14 阶段 1(`scripts/roll.py`)已于 2026-08-05 落地并提交(148bb91)**,下面这段耦合描述的
前置条件——"roll.py 还没落地"——**已不成立**;保留是因为道理没变:P10 新写的表
仍然应该**一出生就调用 `python scripts/roll.py <新表>`**,而不是先手掷用几次再回头接线。
(P16 → P14 的同型耦合已随 P16 完成收进上面依赖图那条注记,不在这里重复。)
...
1. **P10 阶段 1.0 采料**(读 9 段区导言,只填四维抽象格)→ **阶段 1.1–1.2**(提炼稿 +
...
2. **P5 阶段 1**(A 档定位图)——先出原型再定稿惯例,别先写模板。
...
~~**P15 core/ 复查勘误**~~ —— **已完成 2026-08-07,待归档**。神格页读路径两者都做、
`templates/great-old-one.md` 落地、`core/11` 补两条镜像项并升级入边审查、阶段 3 六项
制度动作全部落地,详见计划文件与 `WORKLOG.md` 会话记录。

~~**P16 线索引擎 + 三线索检验**~~ —— **已完成 2026-08-07,待归档**。
`reference/tables/clue-engines.md` 落地并接 `roll.py`、`cult-design-zh.md` §四/§五改口径、
`core/04` 第 5 步六条检验(22 行,守住判据 B 阈值)、`core/05` stall trigger、`core/11`
三查升级,详见计划文件与 `WORKLOG.md` 会话记录。
