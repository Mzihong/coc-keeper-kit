...
| P18 | [compile-module](2026-08-08-compile-module.md) | 编译模组文本:一幕一份、正文线性、网状收进「如何跑这一幕」一表;体例照规则书第十五章(13186–14729 行)。新增 `templates/module.md` + `core/16-compile-module.md` + `compile-module` 技能 | 进行中(阶段 1–3 已提交 `e46bfd8`) | **阶段 4 判据已满足但范围要修正**——实测(`00-campaign-primer.md`,2026-08-08)发生的主要漂移是「重编译了但某节带了旧内容」,mtime/哈希抓不到;暴露出 `core/16`/`core/11` 只防新造事实、不防**已撤回**的事实。**阶段 5 仍阻塞**——等第一幕存在并跑过 |
...
graph LR
    ...
    P18["P18 编译模组<br/>(阶段 1-3 完成,阶段 4 可动,阶段 5 阻塞)"]
```

P5、P14、P18 彼此无依赖,可并行,目前也没有任何计划在等它们。已归档计划(P1–P4、P6、
...
**当前可动的下一条:P18 阶段 4**(2026-08-08 起解除阻塞)。它原本的判据是"先手工编译过一份
真实的模组文本",`campaigns/beyond-the-treeline/module/00-campaign-primer.md` 已经满足这条,
而且**实测结果推翻了阶段 4 原定的做法**:真实发生的主要漂移是「重编译了但某一节把旧内容带了
过来」,时间戳全绿、mtime/哈希抓不到。动手前先读该计划阶段 4 的「实测回填」——那里已经把范围
从"写个脚本"改成了"脚本 + 两处 spec 缺口(`core/16`/`core/11` 只防新造事实,不防**已被撤回**
的事实)"。
...
`<arc>-<slug>.md` 与时间线转写零实测,那部分还是要等第一幕真的跑过。
...
| 3 | **P18** 编译模组 | ★★★★☆ | 一份新模板 + 一份新 spec(`core/16`)+ 技能壳 + 接线;阶段 4 一个可选小脚本 | 唯一要**定义一种新文体**的计划,而且好坏只能靠读一遍成品来判断。第十五章那篇(13186–14729 行)给了体例,但「clock 的阶段 → 按阵营分行的日程表」这段转写没有先例可抄。**阶段 5 卡在一个还不存在的战役幕上**,所以它的验收天然滞后 | ✅ 阶段 1–3 全部完成;**阶段 4 已解除阻塞、可动**(判据"先有一份真实编译产物"由 `00-campaign-primer.md` 满足,但实测把它的范围从"写脚本"改成了"脚本 + 两处 spec 缺口");阶段 5 仍等第一幕跑过 |
