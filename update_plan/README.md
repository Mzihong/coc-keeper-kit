...
| P9 | [monster-templates-traits](2026-08-02-monster-templates-traits.md) | 怪物侧强度标尺:五级阶梯的档位区间、按 malleus 四类重排的 type、数值词条(强化)系统与其负载预算 | 进行中(阶段 A 已完成,阶段 B 待开始) | **阶段 A(标尺与词条)已交付**:`reference/rules/monster-scale.md` + `reference/tables/monster-traits.md` 落盘,`core/07`/`core/02`/`core/11`/`templates/monster.md`/bestiary README 五处接线完成——P4 留的 **X 由此补完**(怪物强度 = 阶梯基线 + 词条负载)。**下一步是阶段 B(索引层)**:条目加 `Serves:` 与 ≤40 字索引摘要为真源,扩 `build-reference-index.py` 生成 `reference/tables/monster-index.md`(进 bundle,是 ChatGPT 链路**唯一**的怪物知识通道),外加回改现有 9 只 bestiary 条目。**阶段 C(神格铺设)**约 6 神格 + 8–10 眷族仍在后面——kit 现在几乎是克苏鲁专用(克苏鲁提及 135 次,哈斯塔 **0** 次) |
...
**P9 阶段 A 已完成,阶段 B 现在可以直接开工**,P5 仍等 Keeper 定风格/范围。

1. ~~**P9 阶段 A(标尺与词条)**~~ — **已完成**(2026-08-03 第七轮)。
   `core/07` 那个占位的 X 已补完
2. **P9 阶段 B(索引层)** — 要改 `build-reference-index.py`,交付后
...
| 2 | **P9 阶段 B** 索引层 | ★★★☆☆ | 改 1 个脚本 + 1 个生成物 + 回改 9 只 + 3 处接线 | 唯一要**改脚本**的部分(解析 header 字段、生成导航表、缺摘要要报错)。回改 9 只是纯体力 | ✅ 现在能动 |
