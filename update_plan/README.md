...
| P18 | [compile-module](2026-08-08-compile-module.md) | 编译模组文本:一幕一份、正文线性、网状收进「如何跑这一幕」一表;体例照规则书第十五章(13186–14729 行)。新增 `templates/module.md` + `core/16-compile-module.md` + `compile-module` 技能 | 待执行 | **阶段 1 无阻塞**;**阶段 5(样张)真阻塞**——等 `beyond-the-treeline` 第一幕存在并跑过 |
...
1. **P18 编译模组**(阶段 1–3) —— 新产物,不动既有产物的正确性;阶段 5 样张必须等
...
| 3 | **P18** 编译模组 | ★★★★☆ | 一份新模板 + 一份新 spec(`core/16`)+ 技能壳 + 接线;阶段 4 一个可选小脚本 | 唯一要**定义一种新文体**的计划,而且好坏只能靠读一遍成品来判断。第十五章那篇(13186–14729 行)给了体例,但「clock 的阶段 → 按阵营分行的日程表」这段转写没有先例可抄。**阶段 5 卡在一个还不存在的战役幕上**,所以它的验收天然滞后 | ⚠️ 阶段 1–3 能动,阶段 5 阻塞 |
