...
| # | 计划 | 范围 | 状态 | 阻塞/等待 |
|---|---|---|---|---|
| P1 | [cult-doc-integration](2026-08-02-cult-doc-integration.md) | 克苏鲁教团 docx 四章提炼进 kit + 敌对势力 intake 问题 + 附加寄放:NPC 互动史惯例(2026-08-02) | 进行中(阶段 0 完成;阶段 1 全部四章均已落盘,内容待提交;剩阶段 2) | 无硬阻塞——阶段 2(敌对势力 intake 问题)可直接开工;第三章 D 第二项(campaign `Threat` 字段)与 E(关系图惯例)留给阶段 2 / P5;附加项(NPC 互动史)无依赖可随时做 |
...
第四章(邪教徒/怪物/造物)**已随阶段 1 其余三章一起落盘**——两个原硬前置(P7、P4)
早已解除,第四章只用了本章自身需要的 7 个新法术,没有等 P7 的完整速查。
P1 剩下的只有阶段 2(敌对势力 intake 问题),没有外部依赖,可以直接开工。
...
~~P1 阶段 1 全部四章~~ —— 已完成/已落盘。剩下的:

1. **P1 阶段 2(敌对势力问题)** — 阶段 1 全部四章已落盘,唯一剩余的子任务,无外部依赖
...
| 6 | **P1** 教团文档整合 | ★★★☆☆(阶段 1 完成后降级) | 阶段 1 全部四章已落盘;剩阶段 2,跨 `core/01`+`campaigns/_template-campaign` | 阶段 1(四章提炼)是本表最重的部分,现已做完;剩下的阶段 2 只是一个 intake 问题 + 一个 campaign 字段,体量已不再是第一名 | ✅ 是 |
