...
**P15(`core/` 复查勘误)已于 2026-08-07 完成并归档(86f1335)**,不再是图上的节点——
`P16 -. 阶段 4 与 P15 阶段 2 撞 core/11 .-> P15` 这条箭头已随 P15 阶段 2 与 P16 阶段 4
双双落地(顺序 P11 → P15 → P16,无冲突)一并删除。

**P16(线索引擎 + 三线索检验)已于 2026-08-07 完成并归档(86f1335)**,不再是图上的节点——
...
这一处——`P14` 做到阶段 1.2 时记得一并补上。

**P10(城镇解剖手法稿)已于 2026-08-07 完成并归档(`7731a01`)**,不再是图上的节点,两条边随之解决,
不是删除:①`P10 == town-institutions 是高频表,该一出生就接 roll.py ==> P14`——
`town-institutions.md` 落地时已直接写成 `python scripts/roll.py town-institutions ...`
最终措辞,通过 `--check-all` 自检,这条耦合描述的风险(新表先脱离脚本被用熟,回头要改接线)
没有发生,**也没有给 `P14` 阶段 1.2 留下待补的掷骰点措辞**——上面 `clue-engines.md` 那笔
遗留欠账不含 P10 这处,此前的预判(两处 `core/03` 掷骰点)偏高,实际只新增了一处,且落地
时已是最终措辞;②`P10 -. 阶段 2 镇级地图卡 = P5 阶段 3 的降级格式 .-> P5`——地图卡格式
已扩进 `diagram-conventions-zh.md` §六,P5 阶段 3 要用时同一文件直接读得到,不必再跨计划
指路,耦合从"两个计划要对齐格式"变成"同一份现成文件"。
...
**P14 阶段 1(`scripts/roll.py`)已于 2026-08-05 落地并提交(148bb91)**。P10 → P14 曾是
这批计划里唯一一条实线耦合(P10 新建的 `tables/town-institutions.md` 是「掷 N 次填清单」
的高频表,该一出生就接 `roll.py`,而不是先手掷用几次再回头接线),已随 P10 完成收进上面
依赖图那条注记,不在这里重复。(P16 → P14 的同型耦合同理,已随 P16 完成收进依赖图注记。)
...
1. **P5 阶段 1**(A 档定位图)——先出原型再定稿惯例,别先写模板。
   ...
   > `diagram-conventions-zh.md` §六 建筑卡本身已随 P10 落地,这里待答的是**另一件事**:
   > ChatGPT 链路本身是否还存在——链路若不存在,阶段 3 才可能整块不必做。

**已完成的历史条目**(2026-08-04,不再占编号):P5 阶段 0(mermaid 惯例落地,还掉
`templates/cult.md` 与 `craft/cult-design-zh.md` 两处指向不存在规范的引用)。

~~**P10 城镇解剖手法稿**~~ —— **已完成并归档(2026-08-07,7731a01)**,见
[Archived/README.md](Archived/README.md)。
...
~~**P15 core/ 复查勘误**~~ —— **已完成并归档(2026-08-07,86f1335)**,见
[Archived/README.md](Archived/README.md)。

~~**P16 线索引擎 + 三线索检验**~~ —— **已完成并归档(2026-08-07,86f1335)**,见
[Archived/README.md](Archived/README.md)。
