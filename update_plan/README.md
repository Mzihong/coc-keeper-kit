...
| P9 | [monster-templates-traits](2026-08-02-monster-templates-traits.md) | 怪物侧强度标尺:五级阶梯的档位区间、按 malleus 四类重排的 type、数值词条(强化)系统与其负载预算 | 阻塞(等 Keeper 完善 malleus 转录稿) | **决策全部拍板(定案 1/2/3/4/5/7/8),卡的是数据源**——Keeper 要重做 `reference/sourcebooks/malleus-monstrorum-zh.md`("等我完善了怪物之锤所有信息都有")。执行清单 4–8、9、10、12、13 其实不依赖它,但 Keeper 已表示先等。17 项分 A/B/C 三阶段。**A 标尺与词条**:词条走数值侧带强度预算、每条强制自带破解口,落 `reference/rules/monster-scale.md` + `reference/tables/monster-traits.md`;type 按 malleus 四类重排、阶梯扩为五级(插入「唯一存在」)——P4 留的 **X 由此补完**。**B 索引层**:条目加 `Serves:` 与 ≤40 字索引摘要为真源,扩 `build-reference-index.py` 生成 `reference/tables/monster-index.md`(进 bundle,是 ChatGPT 链路**唯一**的怪物知识通道)。**C 神格铺设**:约 6 神格 + 8–10 眷族——kit 现在几乎是克苏鲁专用(克苏鲁提及 135 次,哈斯塔 **0** 次)。转载规则已单独落盘(13 个文件):**数值可转,描述不抄、自己总结** |
...
graph LR
    ...
    P9A["P9-A 标尺与词条<br/>(接线部分不卡)"]
    P9B["P9-B 索引层<br/>(机制部分不卡)"]
    P9C["P9-C 神格铺设<br/>(全卡)"]
    KeeperMap["⏳ Keeper 定地图风格/DSL 范围"]
    KeeperMM["⏳ Keeper 完善 malleus 转录稿"]
...
P9 原有的 `Keeper 定来源红线 → P9` 箭头已在 2026-08-03 定案时删除——红线问题已定案
(可抄录:数值可转、描述自己总结)。**取而代之的是一条数据源依赖**:Keeper 要重做
malleus 转录稿,数值区间与神格内容都出自它。**决策上 P9 没有任何等待项。**
...
已落盘。**两个活动计划现在都在等 Keeper**:P5 等风格/范围,P9 等 malleus 转录稿。

## 建议执行顺序

**⚠️ 2026-08-03 起,两个计划都等 Keeper,没有可以立刻开工的计划。** 下面是解除后的顺序。

1. **P9 阶段 A(标尺与词条)** — 转录稿到位后开工。1→2→3 硬串行(抽样定区间 →
   档位表 → 词条负载表),4–8 接线可并行。**4–8 其实不依赖转录稿**,Keeper 若改主意
   要先推进,从这五项起手。交付后 `core/07` 那个占位的 X 就补完了
2. **P9 阶段 B(索引层)** — 等 A。要改 `build-reference-index.py`,交付后
   ChatGPT 链路第一次能看到怪物(`bestiary/` 不进 bundle,导航表是唯一通道)
3. **P9 阶段 C(神格铺设)** — 等 A+B,且**完全依赖转录稿**(神格数据出自这本书)。
   交付后"黄衣之王的精英怪是什么"才真正跑得通
...
|---|---|---|---|---|---|
| 1 | **P9 阶段 A** 标尺与词条 | ★★★☆☆ | 2 个新建 + 6 处接线 | 决策成本已归零,全是实现。重头是清单第 1 项(**人工判读** malleus 抽样定区间);其余是接线 | ⚠️ **接线项(4–8)不卡,数值项(1–3)等转录稿** |
| 2 | **P9 阶段 B** 索引层 | ★★★☆☆ | 改 1 个脚本 + 1 个生成物 + 回改 9 只 + 3 处接线 | 唯一要**改脚本**的部分(解析 header 字段、生成导航表、缺摘要要报错)。回改 9 只是纯体力 | ⚠️ 机制项(9/10/12/13)不卡,校准数值等 A |
| 3 | **P9 阶段 C** 神格铺设 | ★★★★☆ | 约 6 个神格 + 8–10 只眷族 = 14+ 个新文件 | **纯内容写作量**,复杂度不高但体量大,且每只都要过阶段 A 的标尺。kit 现在几乎是克苏鲁专用,这一阶段是拓宽而非补漏 | ❌ 全卡(等转录稿 + A + B) |
