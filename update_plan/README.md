...
6. **P5 阶段 0**(mermaid 惯例落地)——这是**还债不是新功能**:`templates/cult.md` 与
   `craft/cult-design-zh.md` 两个文件正引用一份不存在的规范,而且指向的是 `update_plan/`。
   零阻塞,该优先于 P5 后面几个阶段。(**P13 已落地,这条债的理由已经换了**:不再是
   「bundle 链路拿不到」,而是「计划会归档、链接会烂」——债本身照还。)
7. **P5 阶段 1**(A 档定位图)——先出原型再定稿惯例,别先写模板。
8. **P12 阶段 0 → 1**(速查表自足化)——**三问已定案,现在只等一节样例定体例**。
   优先级高于 P5/P10 剩下的部分,理由是它修的是**已经在牌桌上出错的数值**
   (`chases.md` 的载具 MOV 差一个数量级、`madness.md` 掷 4 号是另一件事),
   而其余计划做的是新功能。**阶段 1.0(改转载规则的边界,即 `WORKLOG.md` 硬约定 5)必须先于阶段 1.1 写表。**
9. **P14 阶段 1**(`scripts/roll.py` + "不许口头掷骰"的硬约定)——**优先级仅次于 P12**,
   理由同样是它修的是**现在就在出错的东西**:模型口头报点数不均匀,种子表的反套路机制
   实际上一直在打折,而且没有任何地方记得下这件事发生过。阶段 1 落地后阶段 2 的两处接线
   是零写作成本,阶段 5 的扩容则可能整个不必做。
