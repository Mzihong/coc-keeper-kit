...
| P18 | [compile-module](2026-08-08-compile-module.md) | 编译模组文本:一幕一份、正文线性、网状收进「如何跑这一幕」一表;体例照规则书第十五章(13186–14729 行)。新增 `templates/module.md` + `core/16-compile-module.md` + `compile-module` 技能 | 进行中(阶段 1–3 已完成,未提交) | 阶段 4 条件执行(先要一份真实编译产物);**阶段 5(样张)真阻塞**——等 `beyond-the-treeline` 第一幕存在并跑过 |
...
graph LR
    ...
    P18["P18 编译模组<br/>(阶段 1-3 已完成,阶段 5 阻塞)"]
```
...
**当前没有可动的下一条**——P5、P14、P18 剩下的都是「等真实使用数据」型收尾,不占排队位:
P5 只剩回填真实 token 成本(等下次真正生成玩家版互动场景图);P14 剩阶段 5(高频表扩容,
判据是"先跑一个真战役")/ 阶段 6(收尾);P18 阶段 1–3 已完成,阶段 4(漂移检查脚本)与
阶段 5(样张验证)都等 `beyond-the-treeline` 第一幕真的跑过。
...
| 3 | **P18** 编译模组 | ★★★★☆ | 一份新模板 + 一份新 spec(`core/16`)+ 技能壳 + 接线;阶段 4 一个可选小脚本 | 唯一要**定义一种新文体**的计划,而且好坏只能靠读一遍成品来判断。第十五章那篇(13186–14729 行)给了体例,但「clock 的阶段 → 按阵营分行的日程表」这段转写没有先例可抄。**阶段 5 卡在一个还不存在的战役幕上**,所以它的验收天然滞后 | ✅ 阶段 1–3 全部完成;阶段 4 条件执行、阶段 5 阻塞,都等 `beyond-the-treeline` 第一幕跑过 |
