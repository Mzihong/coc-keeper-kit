...
| # | 计划 | 范围 | 状态 | 阻塞/等待 |
|---|---|---|---|---|
| P5 | [low-cost-maps](2026-08-02-low-cost-maps.md) | 低成本地图:mermaid 惯例(还债)+ DSL→SVG 渲染器,两个功能——KP 定位图(A 档)与 PL 可互动场景图(A+B 家具层,均已完成) | 进行中(阶段 0-2b 均已完成,阶段 2/2b 于 2026-08-08 提交 `d56c0dc`) | **无阻塞**,唯一未勾的收尾项是"实测功能一/二的 token 差回填 3–5× 估算"——需要一次真实生成才能测,仓库里目前没有在跑的战役,留到 Keeper 下次真正生成互动场景图时回填,不影响计划已交付的能力 |
...
**当前没有排队条目。** P5 已无可动阶段(仅剩一项非阻塞的收尾——回填真实 token 成本,
等下次真正生成互动场景图时再填)。P14 剩阶段 5(高频表扩容)/ 阶段 6(收尾),阶段 5
判据是"先跑一个真战役",仓库里目前没有在跑的战役,条件不满足前不必再排。
