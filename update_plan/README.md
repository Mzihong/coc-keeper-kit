...
| P9 | [monster-templates-traits](2026-08-02-monster-templates-traits.md) | 怪物侧强度标尺:五级阶梯的档位区间、按 malleus 四类重排的 type、数值词条(强化)系统与其负载预算 | 待执行(阻塞已解除,2026-08-03 Keeper 交付重排版转录稿) | **决策全部拍板(定案 1/2/3/4/5/7/8),没有任何等待项**。17 项分 A/B/C 三阶段,依赖顺序 A→B→C。**A 标尺与词条**:词条走数值侧带强度预算、每条强制自带破解口,落 `reference/rules/monster-scale.md` + `reference/tables/monster-traits.md`;type 按 malleus 四类重排、阶梯扩为五级(插入「唯一存在」)——P4 留的 **X 由此补完**。**B 索引层**:条目加 `Serves:` 与 ≤40 字索引摘要为真源,扩 `build-reference-index.py` 生成 `reference/tables/monster-index.md`(进 bundle,是 ChatGPT 链路**唯一**的怪物知识通道)。**C 神格铺设**:约 6 神格 + 8–10 眷族——kit 现在几乎是克苏鲁专用(克苏鲁提及 135 次,哈斯塔 **0** 次)。转载规则已单独落盘:**数值可转,描述不抄、自己总结** |
...
**P9 现在可以直接开工**,P5 仍等 Keeper 定风格/范围。

1. **P9 阶段 A(标尺与词条)** — 1→2→3 硬串行(抽样定区间 →
   档位表 → 词条负载表),4–8 接线可并行。交付后 `core/07` 那个占位的 X 就补完了
...
|---|---|---|---|---|---|
| 1 | **P9 阶段 A** 标尺与词条 | ★★★☆☆ | 2 个新建 + 6 处接线 | 决策成本已归零,全是实现。重头是清单第 1 项(**人工判读** malleus 抽样定区间);其余是接线 | ✅ 现在能动 |
| 2 | **P9 阶段 B** 索引层 | ★★★☆☆ | 改 1 个脚本 + 1 个生成物 + 回改 9 只 + 3 处接线 | 唯一要**改脚本**的部分(解析 header 字段、生成导航表、缺摘要要报错)。回改 9 只是纯体力 | ⚠️ 等 A(数值区间) |
| 3 | **P9 阶段 C** 神格铺设 | ★★★★☆ | 约 6 个神格 + 8–10 只眷族 = 14+ 个新文件 | **纯内容写作量**,复杂度不高但体量大,且每只都要过阶段 A 的标尺。kit 现在几乎是克苏鲁专用,这一阶段是拓宽而非补漏 | ❌ 等 A + B |
